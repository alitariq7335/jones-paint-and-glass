...
const videos = [
    { title: "Video Title", desc: "Description", src: "/assets/videos/1.mp4" },
    { title: "Video Title", desc: "Description", src: "/assets/videos/1.mp4" },
    { title: "Video Title", desc: "Description", src: "/assets/videos/1.mp4" },
    { title: "Video Title", desc: "Description", src: "/assets/videos/1.mp4" },
    { title: "Video Title", desc: "Description", src: "/assets/videos/1.mp4" },
...
export default function VideoSlider() {
    ...
    const max = videos.length - visible;

    // Responsive visible count
    useEffect(() => {
        const update = () => {
            if (window.innerWidth >= 1280) setVisible(4);       // xl
            else if (window.innerWidth >= 768) setVisible(3);   // md
    ...
    useEffect(() => {
        if (current > videos.length - visible) setCurrent(0);
    }, [visible]);
    ...
    return (
        <div className="py-16">
            ...
            <div className="container mx-auto flex items-center justify-between pb-10">
                <h2 className="text-[48px] font-[Avenir] font-bold">Paint</h2>
                <button className="group flex items-center bg-[#A5EBCD] text-black text-[16px] font-semibold px-5 py-3 rounded-[8px] cursor-pointer gap-1">
            ...
            <div className="flex items-center gap-4 container mx-auto">
                ...
                >
                    <svg className="w-8 h-8 stroke-white fill-none" strokeWidth={1.2} viewBox="0 0 24 24">
                    ...
                    >
                        {videos.map((v, i) => (
                            <div key={i} className="flex-shrink-0" style={{ width: cardWidth }}>
                                ...
                                <p className="text-[12px] text-gray-500">{v.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="flex justify-center gap-2 mt-6">
                {Array.from({ length: max + 1 }).map((_, i) => (
                    <button
                        key={i}
                        onClick={() => goTo(i)}
                        className={`h-2 rounded-full transition-all ${i === current ? "bg-[#0052C6] w-4" : "bg-gray-300 w-2"}`}
                    />
                ))}
            </div>
        </div>
