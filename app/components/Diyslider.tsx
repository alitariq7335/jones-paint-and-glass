...
type DiySliderBlockProps = {
  heading?: string
  description?: string
  showSocialTabs?: boolean
  items?: DiyItem[]
}

export default function Diyslider({
  heading = 'Get More DIY Tips & Ideas',
  description = 'Follow us on social media for how-to videos and product info from the same pros who will help you in our stores.',
  showSocialTabs = true,
  items = [],
}: DiySliderBlockProps) {
  ...
  // Debug logging
  console.log('DiySlider items:', items);
            ...
            {items && items.length > 0 ? (
              ...
              >
                {items.map((item, index) => {
                  const imageUrl = item.image?.url ?? '/assets/jt/diy-1.png';
              ...
              <div className="flex items-center justify-center h-[300px]">
                <p className="text-gray-400">No DIY items added. Create items in Payload admin.</p>
              </div>
            )}
          </div>
        </div>
